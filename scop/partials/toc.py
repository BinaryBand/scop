...
class ToCGenerator:
    ...
    def __init__(self, sections: List[Dict[str, Any]] | None = None, *, start_index: int = 4, preamble: List[Dict[str, Any]] | None = None):
    ...
    def from_rfc(cls, model: Any) -> "ToCGenerator":
        ...
        for s in getattr(model, "sections", []) or []:
            ...
            if hasattr(s, "model_dump"):
                ...
                sd = {"title": getattr(s, "title", ""), "subsections": getattr(s, "subsections", [])}
            sections.append(sd)
        ...
        preamble = [
            ...
            {"number": "3", "title": "Design Principles", "anchor": "#3-design-principles"},
        ]
    ...
    def build_nested(self) -> List[Dict[str, Any]]:
        ...
        for idx, sec in enumerate(self.sections, start=self.start_index):
            ...
            node: Dict[str, Any] = {"number": number, "title": title, "anchor": anchor, "children": []}
            ...
            for sidx, sub in enumerate(subsections, start=1):
                ...
                node["children"].append({"number": sub_number, "title": sub_title, "anchor": sub_anchor, "children": []})
            out.append(node)

        # trailing entries: Conformance, Security, IANA, References — compute numbers after sections
        c = self.start_index + len(self.sections)
        out.append({"number": str(c), "title": "Conformance", "anchor": f"#{c}-conformance", "children": []})
        out.append({"number": str(c + 1), "title": "Security Considerations", "anchor": f"#{c+1}-security-considerations", "children": []})
        out.append({"number": str(c + 2), "title": "IANA Considerations", "anchor": f"#{c+2}-iana-considerations", "children": []})
        out.append({"number": str(c + 3), "title": "References", "anchor": f"#{c+3}-references", "children": []})
        return out
